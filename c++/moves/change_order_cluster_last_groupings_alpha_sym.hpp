...
struct change_order_cluster_last_groupings_alpha_sym {
 ...
 change_order_cluster_last_groupings_alpha_sym(configuration *config_, triqs::mc_tools::random_generator* RNG_, int order_max_, int order_min_, double t_max_, double lambda_, double cauchy_t, double cauchy_a, dcomplex diag_):
...
// ----------------------------------------------------------------
 ...
 std::complex<double> attempt(){
  ...
  if (moving_up) {
   ...
   auto flip = config->get_flip_idx(V);

   // Compute second det
   auto pos = std::get<0>(flip);
   V[pos].l = 1 - V[pos].l;
   ...
   if (std::get<1>(flip) && (pos != V.size() - 1)) {
     V[V.size() - 1].l = 1 - V[V.size() - 1].l;
     std::complex<double> det3 = 1;
     for (int sigma = 0; sigma <2; sigma++) det3 *= config->compute_det(sigma, V); 

     V[pos].l = 1 - V[pos].l;
     std::complex<double> det4 = 1;
     for (int sigma = 0; sigma <2; sigma++) det4 *= config->compute_det(sigma, V); 

     V[pos].l = 1 - V[pos].l;
     V[V.size() - 1].l = 1 - V[V.size() - 1].l;
     
     new_det = (det1 + det2 + det3 + det4) / 4;
   }

   else new_det = 0.5 * (det1 + det2);
